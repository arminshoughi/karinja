...
// const job = window.lo
  const getData = () => {
    axios
    .get(`http://127.0.0.1:8001/api/jobs/company/${window.location.pathname.split("/")[2]}/` , {
...
export function useDeleteJobCompany(change) {
  const [collages, setData] = useState([]);
...
// const job = window.lo
  const getData = () => {
    axios
    .delete(`http://127.0.0.1:8001/api/jobs/company/${window.location.pathname.split("/")[2]}/` , {
