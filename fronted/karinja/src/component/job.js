...
function Job() {
  ...
  const { data :apply } = useApply(change);

  console.log(change, "asds");
  ...
  return (
    ...
    <div class="flex items-center space-x-4">
      <button onClick={()=>setChange(true)} class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-lg">Apply Now</button>
      <button class="border-2 border-blue-500 hover:bg-blue-500 text-blue-500 py-2 px-4 rounded-lg">Save Job</button>
    </div>
  </div>
  );
